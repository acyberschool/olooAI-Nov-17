...
const Sidebar: React.FC<SidebarProps> = ({ activeView, setActiveView, isOpen, setIsOpen, isSuperAdmin, permissions }) => {
  ...
  return (
    <>
        ...
        <nav className={`fixed top-0 left-0 h-full w-64 bg-[#F3F4F6] border-r border-brevo-border flex flex-col z-50 transform transition-transform duration-300 ${isOpen ? 'translate-x-0' : '-translate-x-full'} overflow-y-auto shadow-xl`}>
        ...
        <ul className="space-y-1 flex-1 px-2 py-4">
            ...
            {hasAccess('businessLines') && <NavItem label="Business Line" view="businessLines" activeView={activeView} onClick={(v) => { setActiveView(v); setIsOpen(false); }} icon={<BusinessIcon />} />}
            ...
            {hasAccess('sales') && <NavItem label="Sales" view="sales" activeView={activeView} onClick={(v) => { setActiveView(v); setIsOpen(false); }} icon={<SalesIcon />} />}
            ...
            {hasAccess('hr') && <NavItem label="HR" view="hr" activeView={activeView} onClick={(v) => { setActiveView(v); setIsOpen(false); }} icon={<HRIcon />} />}
            
            <div className="my-2 border-t border-gray-200 mx-2"></div>

            {hasAccess('settings') && <NavItem label="Members & Access" view="access" activeView={activeView} onClick={(v) => { setActiveView(v); setIsOpen(false); }} icon={<AccessIcon />} />}
