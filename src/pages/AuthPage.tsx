    ...
    try {
      const { error } = await supabase.auth.signInWithOtp({
        ...
        options: {
          ...
          // IMPORTANT: no emailRedirectTo needed for pure OTP
        },
      });
  ...
  const handleVerifyCode = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.session) {
        ...
        // Force a reload or let App.tsx handle the session change state
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-[#FAF5EF]">
      <div className="w-full max-w-md bg-white rounded-2xl shadow-lg p-8">
        <h1 className="text-2xl font-semibold text-center mb-2 text-[#111827]">
          Welcome to olooAI
        </h1>
          ...
          <form onSubmit={handleSendCode} className="space-y-4">
            <label className="block text-sm font-medium text-gray-700">
              ...
              <input
                ...
                className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-black focus:border-black text-gray-900"
            ...
            <button
              type="submit"
              disabled={loading}
              className="w-full flex items-center justify-center rounded-md bg-black text-white py-2.5 text-sm font-medium disabled:opacity-60 hover:bg-gray-900 transition-colors"
            >
            ...
            <form onSubmit={handleVerifyCode} className="space-y-4">
              <label className="block text-sm font-medium text-gray-700">
                ...
                <input
                  ...
                  className="mt-1 block w-full rounded-md border border-gray-300 px-3 py-2 text-sm tracking-[0.4em] text-center focus:outline-none focus:ring-2 focus:ring-black focus:border-black text-gray-900"
                  placeholder="123456"
                  required
                />
              </label>
